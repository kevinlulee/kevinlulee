...
from kevinlulee.extensions.fancy_file_tree import fancy_file_tree
...
def get_directory_size(path: str, follow_symlinks: bool = False) -> int:
    ...
    has_du_b = subprocess.run(["du", "-b", "/dev/null"],
                              stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode == 0
    if has_du_b:
        out = subprocess.run(["du", "-sb", LFLAG, path], check=True, capture_output=True, text=True).stdout
        return int(out.split()[0])
    else:
        out = subprocess.run(["du", "-sk", LFLAG, path], check=True, capture_output=True, text=True).stdout
        return int(out.split()[0]) * 1024

def get_directory_last_touched(path: str, follow_symlinks: bool = False) -> float:
    path = os.path.expanduser(path)
    LFLAG = "-L" if follow_symlinks else "-H"
    gfind = shutil.which("gfind")
    if gfind:
        # Single-process, very fast
        out = subprocess.run([gfind, LFLAG, path, "-type", "f", "-printf", "%T@\\n",
                              "-o", "-type", "d", "-printf", "%T@\\n"],
    ...
    is_gnu_stat = subprocess.run(["stat", "--version"],
                                 stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode == 0
    find_cmd = ["find", LFLAG, path, "(", "-type", "f", "-o", "-type", "d", ")", "-print0"]
    if is_gnu_stat:
        p1 = subprocess.Popen(find_cmd, stdout=subprocess.PIPE)
        p2 = subprocess.Popen(["xargs", "-0", "-n", "1024", "stat", "-c", "%Y"], stdin=p1.stdout, stdout=subprocess.PIPE)
    else:
        p1 = subprocess.Popen(find_cmd, stdout=subprocess.PIPE)
        p2 = subprocess.Popen(["xargs", "-0", "-n", "1024", "stat", "-f", "%m"], stdin=p1.stdout, stdout=subprocess.PIPE)

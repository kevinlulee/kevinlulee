...
def _normalize_vars(lhs, rhs, variables):
    if variables is None:
        syms = set()
        if lhs is not None:
            syms |= set(lhs.free_symbols)
        if rhs is not None:
            syms |= set(rhs.free_symbols)
    ...
    while True:
        ...
        for values in product(rng, repeat=len(vars_sorted)):
            ...
            if not _passes_constraints(
                assignment, distinct, positive, domain, vars_sorted
            ):
                continue
            if sp.simplify(expr.subs(assignment) - tgt.subs(assignment)) == 0:
        ...
        if radius >= max_radius:
            if deduplicate:
                sols = _deduplicate_solutions(sols, vars_sorted)
            return (
                sols[:max_solutions] if len(sols) > max_solutions else sols,
                radius,
            )
...
def _name_dict(d):
    return {
        str(k): kx.possibly_normalize_number(float(v)) for k, v in d.items()
    }
...
def sympy_solver(
    ...
    Supports variables on either side of '=' (e.g., "a - 5 = 12 - 2a").
    ...
    # collect variables from BOTH sides unless user provided an explicit set
    vars_sorted = _normalize_vars(expr, tgt, variables)
    eq = sp.Eq(expr, tgt)
    ...
    if len(vars_sorted) == 1:
        ...
        if domain == "Z":
            ...
            solset = sp.solveset(eq, v, domain=sp.S.Reals)
            solset = sp.FiniteSet(*[s for s in solset if s.is_rational])
        ...
        if isinstance(solset, sp.FiniteSet):
            ...
            # non-finite sets are not supported by the naming/printing path; return as-is
            return solset
    ...
    if method_use == "search":
        ...
        sols, used_radius = _search_int(
            expr,
            tgt,
            vars_sorted,
            domain,
            only_positive_answers,
            distinct_solutions,
            max_solutions,
            max_radius,
            deduplicate,
